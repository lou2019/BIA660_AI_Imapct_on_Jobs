...
def create_vocab(domain, maxlen=0, vocab_size=0): # 需要决定最大长度, 和vocab = feature space的大小. 同时将统计的资料保存到一个文档
    ...
    source = '/content/drive/My Drive/Attention-Based-Aspect-Extraction-master/preprocessed_data/' + domain + '/train.txt'
    ...
    vocab_file = codecs.open('/content/drive/My Drive/Attention-Based-Aspect-Extraction-master/preprocessed_data/%s/vocab' % domain, mode='w', encoding='utf-8') # 原来是utf8 没有中间短号!!!
...
def read_dataset(domain, phase, vocab, maxlen): # 统计最长长度+字典index对应关系
    ...
    source = '/content/drive/My Drive/Attention-Based-Aspect-Extraction-master/preprocessed_data/' + domain + '/' + phase + '.txt'
